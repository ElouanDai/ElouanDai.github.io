...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | ElouanDai🥝")[0];
}
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| ElouanDai🥝") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('ElouanDai🥝的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(http://43.143.238.78/resource/Shenli.webp);
